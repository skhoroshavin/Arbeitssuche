...
describe("job-site plugins", () => {
  ...
  for (const { name, supportedModes } of getJobSiteInfos()) {
    ...
    test(`${name} (${mode}) - pagination returns unique URLs`, async () => {
      if (skip) return;
      ...
      expect(allUrls.size > 0).toBeTruthy();
    ...
    test(`${name} (${mode}) - vacancy details from Berlin`, async () => {
      if (skip) return;
      ...
      expect(foundBerlinAddress).toBeTruthy();
    }, 60_000);
  }
});
