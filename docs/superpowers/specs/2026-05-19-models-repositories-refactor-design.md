...
Each `createSqlite*Repository(database)` checks its version and runs v0→v1 migration **before** `CREATE TABLE IF NOT EXISTS`. The v0 baseline is the schema from the `v0.2.0` git tag.
...
Set `_migrations` version to 1 for repository `"applicant"`.
...
Set `_migrations` version to 1 for repository `"job-search"`.

#### Vacancy Repository Migration (v0→v1)

No schema changes from v0.2.0. Just set `_migrations` version to 1 for repository `"vacancy"`.
